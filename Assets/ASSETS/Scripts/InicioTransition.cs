...
public class InicioTransition : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(timeToSpawnAnotherCactus < 0){
            ...
            while(rndPosCactus > 1.25f && rndPosCactus < 2.25f)
            ...
            Destroy(cac.GetComponent<lookAtCamera>());
            rb2d.AddForce(Vector2.down * 5f);
        }

        if(empieza) {
            Vector2 camLerp = Vector3.Lerp(cam.transform.position, transform.position, Time.unscaledDeltaTime * 1.5f);
            ...
            for(int i = 0; i < gosToMove.Length; i++){
                gosToMove[i].transform.Translate(Vector3.down * Time.deltaTime * 8);
            }
